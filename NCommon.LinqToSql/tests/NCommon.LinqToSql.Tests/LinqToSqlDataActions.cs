...
namespace NCommon.Data.LinqToSql.Tests
{
    public class LinqToSqlDataActions
    {
        ...
        public SalesTerritory CreateSalesTerritory()
        {
            ...
            {
                Name = "ABC",
                Description = "Test Sales Territory"
            };
            _generator.Context<HRDataDataContext>().SalesTerritories.InsertOnSubmit(territory);
            _generator.EntityDeleteActions.Add(x => ((HRDataDataContext)x).SalesTerritories.DeleteOnSubmit(territory));
            return territory;
        }
    }
}